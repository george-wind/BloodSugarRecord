...
public class PermissionHelper implements OnActivityPermissionCallback {
     ...
     * open android settings screen for the specific package name
     */
    public void openBatteryOptimizationScreen() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            Intent intent = new Intent();
     ...
     * open android settings screen for the specific package name
     */
    public static void openDataOptimizationScreen(Context context){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            try {
                Uri uri = Uri.parse("package:"+context.getPackageName());
                Intent intent = new Intent(Settings.ACTION_IGNORE_BACKGROUND_DATA_RESTRICTIONS_SETTINGS,uri);
                context.startActivity(intent);
            }catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static boolean isNoNeededPermissionRequest(){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
            return false;
        }else{
            return true;
